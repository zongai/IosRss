...
struct ArticleListView: View {
    ...
    private var articles: [Article] {
        ...
        if store.showReadArticles { return all }
    ...
    var body: some View {
        List {
            ForEach(articles) { article in
                ...
                .swipeActions(edge: .leading) {
                    Button { store.toggleFavorite(article) } label: {
        ...
        .navigationDestination(for: Article.self) { article in
            ArticleReaderView(article: article)
                ...
                .onDisappear {
                    if openedArticleID == article.id { openedArticleID = nil }
                    withAnimation(.snappy(duration: 0.25)) { readingIDs.remove(article.id) }
                }
        }
        .toolbar {
            ToolbarItemGroup(placement: .topBarTrailing) {
                ...
                Button { Task { await toggleTranslateAll() } } label: {
                    if isTranslatingAll {
                        HStack(spacing: 5) {
                        ...
                        Image(systemName: "translate")
                            ...
                            .background(showAllTranslations ? Color.primary : Color.secondary.opacity(0.15), in: .rect(cornerRadius: 6))
                    }
                }
                .accessibilityLabel(showAllTranslations ? "显示原文标题" : "翻译列表")
                .disabled(isTranslatingAll)

                Button { store.markAllAsRead(in: feed.id) } label: {
        ...
        .overlay {
            if isInitialLoading {
                VStack(spacing: 12) {
                    ProgressView()
                    Text("正在加载文章…").font(.system(size: 14)).foregroundStyle(.secondary)
                }
        ...
        .task(id: feed.id) {
            ...
            if hasCachedTranslation { showAllTranslations = true }
        }
    }
    ...
    private func toggleTranslateAll() async {
        if showAllTranslations { showAllTranslations = false; return }
        ...
        for batch in jobs.chunked(into: translationBatchSize) {
            ...
            for (job, result) in zip(batch, results) {
                ...
                switch job.field {
                case .title: updates.append((job.articleID, result, nil))
                case .summary: updates.append((job.articleID, nil, result))
                }
            }
            if !updates.isEmpty { store.applyListTranslations(updates) }
            await Task.yield()
        }
        ...
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: 5) {
                HStack(alignment: .top, spacing: 6) {
                    ...
                    if live.isFavorite {
                        Image(systemName: "star.fill")
                            ...
                            .foregroundStyle(.orange).padding(.top, 3)
                    }
                }
                ...
                if !displaySummary.isEmpty {
                    Text(displaySummary).font(.system(size: store.listSummaryFontSize)).foregroundStyle(Color.secondary).lineLimit(2)
                    if showTranslation && store.titleDisplayMode == .bilingual && live.translatedSummary != nil && !live.summary.isEmpty {
                        Text(live.summary).font(.system(size: max(12, store.listSummaryFontSize - 2))).foregroundStyle(Color.secondary.opacity(0.8)).lineLimit(2)
                    }
                }
            }
            .padding(.vertical, 14)

...
enum FeedParser {
    ...
    private static func parseAtom(_ xml: String, feedID: UUID, feedTitle: String) -> [Article] {
        extractBlocks(from: xml, tag: "entry").compactMap { block -> Article? in
            ...
            return Article(
                id: UUID(), feedID: feedID, feedTitle: feedTitle,
                title: stripHTML(title), link: link,
    ...
    static func extractTag(_ tag: String, from xml: String) -> String? {
        ...
        guard let openRange = xml.range(of: openPat, options: [.regularExpression, .caseInsensitive]),
              let closeRange = xml.range(of: closePat, options: [.regularExpression, .caseInsensitive], range: openRange.upperBound..<xml.endIndex) else { return nil }
    ...
    static func parseDate(_ str: String) -> Date? {
        ...
        let formatters = ["EEE, dd MMM yyyy HH:mm:ss Z", "EEE, dd MMM yyyy HH:mm:ss zzz", "yyyy-MM-dd'T'HH:mm:ssZ", "yyyy-MM-dd'T'HH:mm:ss.SSSZ", "yyyy-MM-dd'T'HH:mm:ssXXXXX", "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd"]
...
struct FeedDiscovery {
    static func discoverFeeds(from url: URL) async throws -> [DiscoveredFeed] {
        ...
        if found.isEmpty { found = guessCommonFeedPaths(baseURL: finalURL) }
        return found
    }
    private static func parseAlternateFeedLinks(from html: String, baseURL: URL) -> [DiscoveredFeed] {
        ...
        for match in regex.matches(in: html, range: NSRange(html.startIndex..., in: html)) {
            ...
            guard let type = extractAttr("type", from: tag), (type.contains("rss") || type.contains("atom")),
            ...
            feeds.append(DiscoveredFeed(title: FeedNaming.resolveTitle(parsed: extractAttr("title", from: tag), url: resolved), url: resolved))
        }
        return feeds
    }
    private static func guessCommonFeedPaths(baseURL: URL) -> [DiscoveredFeed] {
        ["/feed", "/feed/", "/rss", "/rss.xml", "/atom.xml", "/feed.xml", "/index.xml"].compactMap { path in
            guard let url = URL(string: path, relativeTo: baseURL)?.absoluteURL else { return nil }
            return DiscoveredFeed(title: FeedNaming.domainName(from: baseURL.absoluteString), url: url.absoluteString)
        }
    }
...
struct OPMLParser {
    ...
    func parse() -> [OPMLItem] {
        ...
        for match in regex.matches(in: xml, range: NSRange(xml.startIndex..., in: xml)) {
            ...
            let rawTitle = extractAttr("text", from: tag) ?? extractAttr("title", from: tag) ?? extractAttr("description", from: tag)
            items.append(OPMLItem(title: FeedNaming.resolveTitle(parsed: rawTitle.map { HTMLUtils.decodeEntities($0) }, url: url), url: url))
        }
    ...
    private func firstFeedURL(in tag: String) -> String? {
        for key in ["xmlUrl", "xmlurl", "xmlURL", "XMLUrl", "xml_url", "xmluri", "xmlUri"] {
        ...
        if let v = extractAttr("htmlUrl", from: tag) ?? extractAttr("htmlurl", from: tag), looksLikeFeedURL(v) { return v }
        return nil
    }
    ...
    private func looksLikeFeedURL(_ s: String) -> Bool {
        ...
        let t = s.lowercased()
        return t.contains("rss") || t.contains("atom") || t.contains("feed") || t.contains(".xml") || t.contains("/rdf") || t.contains("syndication")
    }

    private func extractAttr(_ attr: String, from tag: String) -> String? {
        let pattern = "\(attr)\\s*=\\s*(?:\"([^\"]*)\"|'([^']*)'|([^\\s>]+)"
        // BUG - need proper pattern
        let p2 = attr + "\\s*=\\s*(?:\"([^\"]*)\"|'([^']*)'|([^\\s>]+)"
        guard let regex = try? NSRegularExpression(pattern: "\(attr)\\s*=\\s*(?:\"([^\"]*)\"|'([^']*)'|([^\\s>]+))", options: .caseInsensitive),

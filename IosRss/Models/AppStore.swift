...
class AppStore {
    ...
    func exportOPML() -> String {
        ...
        for feed in feeds {
            let t = feed.title.replacingOccurrences(of: "\"", with: """)
            xml += "    <outline type=\"rss\" text=\"\(t)\" xmlUrl=\"\(feed.url)\"/>\n"
        }
        xml += "  </body>\n</opml>"
        return xml
    }
    ...
    private func loadFromStorage() {
        ...
        if let mode = ud.string(forKey: "titleDisplayMode"), let m = TitleDisplayMode(rawValue: mode) {
            titleDisplayMode = m
        }
        if let eng = ud.string(forKey: "defaultTranslationEngine"), let e = TranslationEngine(rawValue: eng) {

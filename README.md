...
**版本**：本地调试 `v1.3-14`；CI 构建 `v1.3-14-build{N}`（`N` = GitHub Actions `run_number`）。
...
- 每个 AI Provider 可单独测试连接；设置导出默认不含 Key（可选手动包含）
- API Key 使用系统钥匙串；网络请求拒绝本机/内网地址

### 外观与字体
- **外观**：跟随系统（默认）/ 浅色 / 深色
- **阅读主题**：Classic / Sepia Paper / Night Dark / Midnight Blue / Forest Sage / High Contrast（`ReadingTheme`，随系统明暗自动匹配）
...
|------|------|
| 本地 Xcode | `v1.3-14` |
| GitHub Actions | `v1.3-14-build{N}` |

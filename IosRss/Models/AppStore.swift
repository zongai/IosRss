...
class AppStore {
    ...
    func fetchFullContent(for article: Article) async throws -> Article {
        ...
        let html = try await ArticleContentFetcher.fetch(url: article.link)
        var updated = article
    ...
    func importSubscriptions(data: Data) -> SubscriptionImportResult {
        ...
        if !opmlItems.isEmpty {
            ...
            return SubscriptionImportResult(added: added, skipped: skipped, failed: 0)
        }
        if let feed = FeedParser.parseFeedXML(data: data) {
            let url = FeedURL.canonical(feed.url)
            if feeds.contains(where: { FeedURL.canonical($0.url) == url }) {
                return SubscriptionImportResult(added: 0, skipped: 1, failed: 0)
            }
            feeds.append(feed)
            saveToStorage()
            return SubscriptionImportResult(added: 1, skipped: 0, failed: 0)
        }
        return SubscriptionImportResult(added: 0, skipped: 0, failed: 1)
    }

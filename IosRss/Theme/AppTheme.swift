...
// MARK: - Typography (Inter Tight)

enum AppTypography {
    /// 问候 / 大标题 26
    static func greeting() -> Font {
        font(size: 26, weight: .semibold)
    }

    /// 页面标题 22/28 半粗，字距 -0.7
    ...
    static func font(size: CGFloat, weight: Font.Weight) -> Font {
        ...
        case .semibold, .bold, .heavy, .black:
            name = "InterTight-SemiBold"
        case .medium:
            name = "InterTight-Medium"
        default:
...
extension View {
    ...
    func appSectionStyle() -> some View {
        self
            ...
            .tracking(AppTypography.sectionTracking)
    }
...
enum AppFontRegistration {
    static func register() {
        ...
        for name in names {
            guard let url = Bundle.main.url(forResource: name.replacingOccurrences(of: ".ttf", with: ""), withExtension: "ttf")
                    ?? Bundle.main.url(forResource: name, withExtension: nil)
                    ?? Bundle.main.url(forResource: (name as NSString).deletingPathExtension, withExtension: "ttf", subdirectory: "Fonts")
            else {
                // Try Fonts/ subfolder via path enumeration
        ...
        // Also register any Fonts directory files

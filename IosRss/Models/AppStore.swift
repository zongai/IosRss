...
class AppStore {
    ...
    func resolveAIProvider(preferredID: UUID?, forText text: String) -> AIProvider? {
        let preferred = preferredID.flatMap { id in aiProviders.first(where: { $0.id == id }) }
            ?? aiProviders.first
    ...
    func explainText(_ text: String) async throws -> String {
        guard let provider = resolveAIProvider(
            preferredID: defaultSummaryProviderID ?? defaultTranslationProviderID,
            forText: text
        ) else {
            throw TranslationError.noProvider
        }
    ...
    func fetchFullContent(for article: Article) async throws -> Article {
        ...
        let html = try await ArticleContentFetcher.fetch(urlString: article.link)
        var updated = article
        updated.content = html
        updated.hasFullContent = true
        OfflineCache.saveArticleHTML(link: article.link, html: html)
        updateArticle(updated)
        return updated
    }

    func clearOfflineContentCache() {
        OfflineCache.clearContentCaches()
    }

    func cacheSizeDescription() -> String {
        OfflineCache.formattedCacheSize()
    }

    // MARK: - OPML / TXT export & import (subset kept for compile)

    func exportOPML() -> String {
        var lines: [String] = [
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
            "<opml version=\"2.0\">",
            "  <head><title>IosRss Subscriptions</title></head>",
            "  <body>"
        ]
    ...
    func writeExportFile(filename: String, content: String) -> URL? {
    ...
    func importSubscriptions(from url: URL) throws -> SubscriptionImportResult {
        let data = try Data(contentsOf: url)
        let text = String(data: data, encoding: .utf8)
            ?? String(data: data, encoding: .utf16)
            ?? ""
        let existing = Set(feeds.map { Self.canonicalLink($0.url) })
        if text.lowercased().contains("<opml") || text.lowercased().contains("xmlurl") {
            let items = FeedParser.parseOPML(data)
            var added = 0
            var skipped = 0
            for item in items {
                ...
                if let gname = item.groupName, !gname.isEmpty {
                    if let g = groups.first(where: { $0.name == gname }) {
                        feed.groupID = g.id
                    } else {
                        let order = (groups.map(\.sortOrder).max() ?? -1) + 1
        ...
        for line in text.components(separatedBy: .newlines) {
            ...
            guard t.hasPrefix("http") else { continue }
            let key = Self.canonicalLink(t)
            if existing.contains(key) { skipped += 1; continue }
            feeds.append(RSSFeed(title: key, url: t))
            added += 1
        }
        if added > 0 { saveToStorage() }
        return SubscriptionImportResult(added: added, skipped: skipped, kind: "rss")
    }
    ...
    func saveToStorage() {
        ...
        if let data = try? JSONEncoder().encode(aiBlacklistTerms) {
            UserDefaults.standard.set(data, forKey: "aiBlacklistTerms")
        }

...
class AppStore {
    ...
    /// 已读文章保留天数（非收藏），超过后从列表清理
    var readRetentionDays: Int = 7
    /// 全文 HTML 磁盘缓存保留天数（按写入时间）
    ...
    /// 已读文章链接（规范化），用于刷新后恢复已读状态，避免再次出现为未读
    ...
    @discardableResult
    func importSubscriptions(data: Data) -> Int {
        let feedsToAdd = FeedParser.parseOPML(data: data)
        var added = 0
        let existing = Set(feeds.map { $0.url.lowercased() })
        for f in feedsToAdd {
            if existing.contains(f.url.lowercased()) { continue }
            feeds.append(f)
            added += 1
        }
    ...
    private func loadFromStorage() {
        loadReadLinks()
        if let decoded = OfflineCache.loadFeeds() {
            feeds = decoded
        }
        ...
        let fs = ud.double(forKey: "fontSize")
        fontSize = fs.isZero ? 17 : fs
        listTitleFontSize = ud.double(forKey: "listTitleFontSize").isZero ? 18 : ud.double(forKey: "listTitleFontSize")
        listSummaryFontSize = ud.double(forKey: "listSummaryFontSize").isZero ? 15 : ud.double(forKey: "listSummaryFontSize")
        readerTitleFontSize = ud.double(forKey: "readerTitleFontSize").isZero ? 24 : ud.double(forKey: "readerTitleFontSize")
        aiSummaryFontSize = ud.double(forKey: "aiSummaryFontSize").isZero ? 22 : ud.double(forKey: "aiSummaryFontSize")
        feedTitleFontSize = ud.double(forKey: "feedTitleFontSize").isZero ? 17 : ud.double(forKey: "feedTitleFontSize")
        let rd = ud.integer(forKey: "readRetentionDays")
        readRetentionDays = ud.object(forKey: "readRetentionDays") == nil ? 7 : max(0, rd)
        let cd = ud.integer(forKey: "fullContentCacheDays")
        fullContentCacheDays = ud.object(forKey: "fullContentCacheDays") == nil ? 30 : max(0, cd)
        if let mode = ud.string(forKey: "titleDisplayMode"),
           let m = TitleDisplayMode(rawValue: mode) {
            titleDisplayMode = m
        }
        if let eng = ud.string(forKey: "defaultTranslationEngine"),
           let e = TranslationEngine(rawValue: eng) {
            defaultTranslationEngine = e
        }
        showReadArticles = ud.bool(forKey: "showReadArticles")
        if let p = ud.string(forKey: "translationPrompt"), !p.isEmpty { translationPrompt = p }
        if let p = ud.string(forKey: "summaryPrompt"), !p.isEmpty { summaryPrompt = p }
        if let s = ud.string(forKey: "defaultSummaryProviderID"), let id = UUID(uuidString: s) {
            defaultSummaryProviderID = id
        }
        if let s = ud.string(forKey: "defaultTranslationProviderID"), let id = UUID(uuidString: s) {
            defaultTranslationProviderID = id
        }
    }

    private func seedSampleData() {
        // optional sample; keep empty for production-like start
    }
}
